...
const sendEmail = async (reqBody) => {
    ...
    if (reqBody.commentType === 0) {
    ...
    } else if (reqBody.commentType === 1 || reqBody.commentType === 2) {
        if (reqBody.to) {
            try {
                const toComment = await Comment.findById(reqBody.to);
                if (toComment) {
                ...
                <body>
                    <div class="reply-container">
                        ...
                        <div class="reply-content-box">
                            <div class="reply-content-header">
                                Hi ${toComment.from}，您曾在《<a href="${blogInfo.blogUrl}/${reqBody.commentType === 1 ? 'message' : 'aboutMessage'}"
                                target="_blank">${blogInfo.blogName}</a>》上发表留言：
                            </div>
                            <div class="reply-content-text">
                                ${toComment.content}
                            </div>
                            ...
                            <div class="reply-content-bottom">
                                您可以点击<a href="${blogInfo.blogUrl}/${reqBody.commentType === 1 ? 'message' : 'aboutMessage'}"
                                    target="_blank">查看回复的完整内容</a>，欢迎再次光临${blogInfo.blogName}
                            </div>
                            <div class="replay-desc">
                                本邮件为系统自动发送，请勿直接回复邮件，可到博文内容回复。
        ...
        <body>
            <div class="reply-container">
                <div class="reply-c-box">
                    <div class="reply-container-header">
                        您在<a href="${blogInfo.blogUrl}" target="_blank">[${blogInfo.blogName}]</a>的留言板有新评论！
                    </div>
                    <div class="reply-content-box">
                        <div>
                            ${reqBody.from}，在您的《<a href="${blogInfo.blogUrl}/${reqBody.commentType === 1 ? 'message' : 'aboutMessage'}"
                                target="_blank">${blogInfo.blogName}</a>》上发表留言：
                        </div>
                        <div class="reply-content-text">
                            ${reqBody.content}
        ...
        `;
            ...
            transporter.sendMail({
                from: 'emperor_ywh@163.com',
                to: '1289978696@qq.com',
                subject: `您在[${blogInfo.blogName}]的留言板有新评论！`,
                html: bloggerTemplate
            }, function (err, info) {
                if (err) {
                    console.log('邮件发送失败', err)
